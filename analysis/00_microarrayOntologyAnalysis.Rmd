...
```
```{r}
sig_terms_array <- topGSA(gst, n = 1000) %>% filter(FDR < 0.01)
sig_terms_array
```
...
```{r}
gst.biased <- gometh(sig.cpg=sigCpGs, all.cpg=rownames(top), collection="GO",
              prior.prob=FALSE)

sig_terms_array_biased <- topGSA(gst.biased, n=10000) %>% filter(FDR < 0.01)
...
```{r}
gst_promoter <- gometh(sig.cpg=sigCpGs, all.cpg=rownames(top), collection="GO",
...
```{r}
gst_promoter.biased <- gometh(sig.cpg=sigCpGs, all.cpg=rownames(top), collection="GO",
