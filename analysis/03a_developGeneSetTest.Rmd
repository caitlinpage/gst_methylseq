...
```
- we don't want this - only good for small bits - b/c it reps 1000
```{r}
bench_steps <- microbenchmark(
  genes = sourceGenes(bsseq_res, "biomart", "gene"),
 # genes_eh = sourceGenes(bsseq_res, "experimenthub", "gene"),
  anno = annoGeneDmr(bsseq_res, bsseq_dmrs[1:1000,], "biomart", test_fn_res$genes),
  out = run_miss_methyl_a(test_fn_res$genes, test_fn_res$anno),
  res = run_miss_methyl_b(test_out_again, correct_bias = TRUE)
)
```
```{r}
...
test_loop <- run_miss_methyl_a(biomart_genes, test_anno)
```
```{r}
rep(test_loop$entrez_id, test_loop$bias)
```
```{r}
fn <- function(test_uni_freq) {
...
system.time(fn(test_loop))
```


```{r}

  freq_make_table <- rep(test_uni_freq$entrez_id, test_uni_freq$bias)
...
entrez_genes <- AnnotationDbi::select(org.Hs.eg.db, 
                                ...
                                columns=c("ENTREZID","SYMBOL"), 
                                keytype="ENTREZID")
entrez_genes[fmatch(biomart_genes$gene_name, entrez_genes$SYMBOL), "ENTREZID"]
```
```{r}
microbenchmark(match = entrez_genes[match(biomart_genes$gene_name, entrez_genes$SYMBOL), "ENTREZID"],
               fmatch = entrez_genes[fmatch(biomart_genes$gene_name, entrez_genes$SYMBOL), "ENTREZID"])
```
```{r}
test_anno[test_anno$ensembl_gene_id %fin% biomart_genes$ensembl_gene_id,]
```

```{r}
microbenchmark(test_in = test_anno[test_anno$ensembl_gene_id %in% biomart_genes$ensembl_gene_id,],
               test_fin = test_anno[test_anno$ensembl_gene_id %fin% biomart_genes$ensembl_gene_id,])
```
```{r}
test_anno$entrez_id <- entrez_genes[match(test_anno$gene_name, entrez_genes$SYMBOL), "ENTREZID"]
```
```{r}
microbenchmark(test_in = test_anno[!test_anno$entrez_id %in% NA,],
               fn = test_anno[!is.na(test_anno$entrez_id),])
```
