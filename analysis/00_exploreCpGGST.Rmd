...
```{r}
unique(go_entrez$go) %in% mm_go$GOID %>% summary()
```
* same terms

```{r}
go_entrez <- go_entrez %>% mutate(term = mm_go[match(.$go, mm_go$GOID), "TERM"],
                     ontology = mm_go[match(.$go, mm_go$GOID), "ONTOLOGY"])
go_entrez <- go_entrez %>% group_by(go) %>% mutate(num_genes = n()) %>% ungroup()
```

```{r}
go_entrez_summ <- go_entrez %>% distinct(go, ontology, term, num_genes)
```


```{r}
go_entrez_summ %>%
  ggplot(aes(x = log10(num_genes), colour = ontology)) +
  geom_density()
go_entrez_summ %>%
  ggplot(aes(x = ontology, y = log10(num_genes))) +
  geom_boxplot()
```
