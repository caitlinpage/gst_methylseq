...
```{r}
go_entrez <- go_entrez %>% mutate(num_cg = data.frame(out$freq)[match(.$entrez, data.frame(out$freq)$eg.all), "Freq"])
go_entrez
```
...
```{r}
go_entrez <- go_entrez %>% mutate(mm_seq_bias = ifelse(go %in% mm_hack_null$GOID, TRUE, FALSE))
```
