...
```{r}
go_entrez_bias <- go_entrez %>% mutate(width = txdb_genes[match(.$entrez, txdb_genes$gene_id), "width"],
                                       num_cg = txdb_genes[match(.$entrez, txdb_genes$gene_id), "num_cg"])
go_entrez_bias <- go_entrez_bias %>% filter(!is.na(width), !is.na(num_cg)) %>% group_by(go) %>% mutate(av_width = mean(width), av_num_cg = mean(num_cg)) %>% ungroup()
go_entrez_bias %>% distinct(go, av_width, av_num_cg) %>% melt(id.vars = "go") %>%
