...
```{r}
truth_gst_100 <- rbind(
  sig_terms_array[1:100,] %>% mutate(rank_fdr = 1:nrow(.), in_imm = cumsum(rownames(.) %in% immuneGO$GOID), method = "Wall_1") %>% .[names(.) %in% c("rank_fdr", "in_imm", "method")],
  sig_terms_array_biased[1:100,] %>% mutate(rank_fdr = 1:nrow(.), in_imm = cumsum(rownames(.) %in% immuneGO$GOID), method = "Hyper_1") %>% .[names(.) %in% c("rank_fdr", "in_imm", "method")])

truth_gst_100 %>%
  ggplot(aes(x = rank_fdr, y = in_imm, colour = method)) +
  geom_line(size = 2) +
  labs(x = "Rank", y = "Cumulative no. in immune sets", title = "Array") +
  theme_bw()
```
# Use array results as ground truth
```{r}
truth_gst_100 <- rbind(
